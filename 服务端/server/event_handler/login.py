...
def run(sc, parameters):
    ...
    r = c.execute('SELECT id,username,faceid from users where username=? and password=?', (parameters[0], md5(parameters[1])))
    rows = r.fetchall()
    ...
    sc.send(MessageType.login_successful, [user,face_id])
